...
void RC5Simple::RC5_DecryptFileRQFF(unsigned char *in_name, vector<unsigned char> &out_name)
{
	RC5_DecFileForRQFF(in_name, out_name);
}

void RC5Simple::RC5_DecryptFileRQFF(const char *in_name, vector<unsigned char> &out_name)
{
	RC5_DecFileForRQFF((unsigned char *)in_name, out_name);
}
...
void RC5Simple::RC5_DecFileForRQFF(unsigned char *in_name, vector<unsigned char> &out)
{
	FILE *in_file;
	...
	// Возвращаемся к начаялу файла
	fseek(in_file, 0, SEEK_SET);

	// Создаём вектор данных
	vector<unsigned char> in(in_file_length);

	// Заполнение вектора данных
	unsigned int byte = 0;
	while (byte<in_file_length)
		in[byte++] = fgetc(in_file);
	...
	RC5_Decrypt(in, out); // Расшифровка вектора данных
}
